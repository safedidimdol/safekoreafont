...
//**************************************************************************
					...
					if(cnt>0 && totCnt > 0){
						...
						for(var i=0;i<cnt;i++){
							...
							if(_.isEmpty(json.SHEL_DIV_TYPE)){	
								gen.getChild( i , "a1" ).setValue( '-' );
							}
							if(_.isEmpty(json.TEL)){	
								gen.getChild( i , "a5" ).setValue( '-' );
							}
						}
					}
					else{
		...
		function fn_openPop(row){
			...
			addr = addr.split(',')[0];
	 		...
	 		// 주소검색
			var url = "/idsiSFK/neo/mapViewPopup_neo.do?addr="+encodeURI(encodeURIComponent(addr))+"&facilNm="+encodeURI(encodeURIComponent(facilNm));
		...
		function fn_openPop2(row){
			...
			if( (lo != "" && lo != undefined) && (la != "" && la != undefined) ){
			  	url = "/idsiSFK/neo/mapViewPopup_neo.do?lo="+json.LON+"&la="+json.LAT+"&facilNm="+encodeURI(encodeURIComponent(facilNm));
			} else {
				url = "/idsiSFK/neo/mapViewPopup_neo.do?addr="+encodeURI(encodeURIComponent(addr))+"&facilNm="+encodeURI(encodeURIComponent(facilNm));
			}
		    window.open( url, "spamMain", "height=660px; width=660px; left=0; top=0; toolbar=no, menubar=no, location=no, status=yes, realzable=no, fullscreen=no, scrollbars=no, resizable=yes");
		}
