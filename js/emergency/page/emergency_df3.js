...
var $li = $('#gen').find('li').clone();
	...
	function fn_search () {
		...
		if(num==1){
			...
			readJSONFile("waterList", sgg.ORG_CD);
	...
	function setHTMLList() {
		...
		for(i = 0; i < length; i++) {
			...
			var $cell1 = $li.find('#a1'); 
			var $cell2 = $li.find('#a2');
			var $cell3 = $li.find('#a3'); 
			...
			if(mapIcon) {		
				...
				if(jdata[10*(pageIndex-1)+i].FACIL_RD_ADDR != null){
					var rdAddr = "<a class='ellipsis mapView_link' href='javascript:fn_openPop("+i+")'>도로명 : " +  '<i class="ri-map-pin-2-line"></i>'  + facil_rd_addr + "<br/>"; 
					var bdAddr = "법정동 : " + '<i class="ri-map-pin-2-line"></i>' + facil_addr
			...
			innerHTML = jdata[10*(pageIndex-1)+i].FACIL_NM + "<br/>(" + jdata[10*(pageIndex-1)+i].FACIL_GBN_NM + ")";
	...
	function fn_openPop(row){
		...
		*/
 		...
 		url = "/idsiSFK/neo/mapViewPopup_neo_cdn.do?lo="+lo+"&la="+la+"&facilNm="+encodeURI(encodeURIComponent(param.facilNm)) + '' +  "&facilDtl="+encodeURI(encodeURIComponent(param.facilDtl));
