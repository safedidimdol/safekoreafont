		...
		for(i = 0; i < length; i++) {
			...
			if(mapIcon) {		
				...
				if(jdata[10*(pageIndex-1)+i].FACIL_RD_ADDR != null){
					...
					$celln1.html(rdAddr);
					if(_.isEmpty(jdata[10*(pageIndex-1)+i].FACIL_RD_ADDR)){		
						$celln1.parents('.col4').css('display','none');
					}else{
						$celln1.parents('.col4').css('display','');
					}
					$celln2.html(bdAddr);
			...
			$celln3.html(jdata[10*(pageIndex-1)+i].FACIL_GBN_NM);
